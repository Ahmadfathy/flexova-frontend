...
**تطبيق فرونت-إند:** `stock_batch` بيسجّل `item_id` دايماً؛ `variant_id` بيتحطّ بس لو الدفعة تابعة لمتغيّر DD-1 حقيقي، وإلا `null`. حامل الرصيد = `coalesce(variant_id, item_id)` — دالة واحدة `balanceCarrier()` في `items/batches.ts` بتحسبه، وكل من مفتاح الدمج ومحرّك الاختيار بيستخدموها (مش `variant_id` لوحده على طول). مفيش كيان "متغيّر افتراضي" منفصل اتاخترع للأصناف البسيطة على الفرونت — نفس منطق الباك-إند إن `item_variant.variant_of` بيرجع لـ `item_id` افتراضياً، بس من غير حاجة لصف variant فعلي في fixture الموك.
