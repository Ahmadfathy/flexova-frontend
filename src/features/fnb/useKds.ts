...
/**
 * Five-states signal for KDS (`?mock=loading|empty|error|offline`), mirroring
 * `useFloorPlan` — tickets themselves are read live from `useFnbKds` (a
 * persisted store), this hook only decides which of the five states wraps
 * that live data.
 */
