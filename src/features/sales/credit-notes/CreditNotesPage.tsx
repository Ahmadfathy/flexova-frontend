...
// ── New Credit Note Sheet ─────────────────────────────────────────

interface NewCreditNoteSheetProps {
  open: boolean;
  onOpenChange: (v: boolean) => void;
  defaultSourceId?: string;
}

function NewCreditNoteSheet({ open, onOpenChange, defaultSourceId }: NewCreditNoteSheetProps) {
      ...
      <PageSection padded={false}>
        {creditNotes.length === 0 ? (
          <EmptyState
            ...
            action={canCreate ? { label: t("credit.new"), onClick: () => setSheetOpen(true) } : undefined}
          />
        ) : (
          <Table>
            <TableHeader>
