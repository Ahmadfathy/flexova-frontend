...
/** Five-states signal for the rate-plans list (`?mock=loading|empty|error|no_results|offline`),
 * mirroring `useDeviceTypesList` — rate plans are read live from `usePlayRatePlans`, this hook
 * only decides which wrapper-state to render around that live data. */
export function useRatePlansList() {
