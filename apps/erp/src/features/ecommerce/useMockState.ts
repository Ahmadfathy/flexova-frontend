...
/**
 * Shared five-states signal (`?mock=loading|empty|error|offline`) for
 * E-commerce Admin screens whose data is read live from a store/fixture
 * (not fetched) — mirrors Healthcare/Projects' `useMockState`.
 * `no_results` is always derived by the page itself (filters yielding zero rows).
 */
