...
/** Five-states signal for the Floor Grid (`?mock=loading|empty|error|no_results|offline`),
 * mirroring `useFloorPlan`/`useDeviceTypesList` — devices/sessions are read live from their
 * stores, this hook only decides which wrapper-state to render around that live data. */
export function useFloorGrid() {
